 ...
 */
const markdownFiles: ArticleMarkdownMap = import.meta.glob(
  '/src/content/blog/**/*.md',
  {
    as: 'raw',
    eager: false,
...
export async function loadArticleContent(
  ...
  // Try to load markdown file for the requested locale
  const localePath = `/src/content/blog/${article.slug}/${locale}.md`;
  const fallbackPath = `/src/content/blog/${article.slug}/en.md`;

  try {
    // Try requested locale
    if (markdownFiles[localePath]) {
      const content = await markdownFiles[localePath]();
      // When using 'as: raw', Vite returns the string directly, not in a default property
      return content as unknown as string;
    }

    // Fallback to English if not the requested locale
    if (locale !== 'en' && markdownFiles[fallbackPath]) {
      console.warn(
        `Markdown file not found for ${article.slug} in ${locale}, falling back to English`
      );
      const content = await markdownFiles[fallbackPath]();
      return content as unknown as string;
    }
 ...
 */
export function hasMarkdownFile(slug: string, locale: SupportedLocale): boolean {
  const path = `/src/content/blog/${slug}/${locale}.md`;
  return path in markdownFiles;
}
...
export function getAvailableLocales(slug: string): SupportedLocale[] {
  const pattern = `/src/content/blog/${slug}/`;
  const availableLocales: SupportedLocale[] = [];

  Object.keys(markdownFiles).forEach((path) => {
    if (path.startsWith(pattern)) {
      const locale = path.replace(pattern, '').replace('.md', '') as SupportedLocale;
